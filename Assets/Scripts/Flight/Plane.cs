...
//
// Additional functionality added by Larabee Aka Will Vickers.
//
...
public class Plane : MonoBehaviour
{
    ...
    [SerializeField] private float TurningMultiplier = 1.25f;
    [SerializeField] private float ThrottleSenstivity = 10f;
    ...
    private void Update()
    {
        ...
        if (Input.GetButton("Left Shift") && ThrottlePosition != 100f)
        {
            ...
            {
                ThrottlePosition = ThrottlePosition + ThrottleSenstivity * Time.deltaTime;
            }
        ...
        else if (Input.GetButton("Left Control") && ThrottlePosition != 0f)
        {
            ThrottlePosition = ThrottlePosition - ThrottleSenstivity * Time.deltaTime;
            if (ThrottlePosition < 1f )
            {
        ...
        float autoRoll = 0f;
        if (controller != null)
            if (controller.TurretAimingMode)
                RunAutopilot(controller.BoresightPos, out autoYaw, out autoPitch, out autoRoll);
            else
    ...
    private void FixedUpdate()
    {
        ...
        rigid.AddRelativeTorque((new Vector3(turnTorque.x * pitch, turnTorque.y * yaw, -turnTorque.z * roll) * (ThrottlePosition/100 * TurningMultiplier)) * forceMult, ForceMode.Force);
    }
}
