...
public class MouseFlightController : MonoBehaviour
{
    ...
    public CameraManger BuiltInController;
    ...
    public bool TurretAimingMode = false;
    ...
    public Vector3 MouseAimPos
    {
        get
        {
            if (mouseAim != null)
            {
                return isMouseAimFrozen ? GetFrozenMouseAimPos() : mouseAim.position + (mouseAim.forward * aimDistance);
            }
            else
            {
    ...
    private void Update()
    {
        ...
        RotateRig();
        BuiltInController.DollyCamera();
        if (Input.GetButtonDown("Tab Button"))
        {
            if (TurretAimingMode)
            {
                TurretAimingMode = false;
            }
            else
            {
                TurretAimingMode = true;
            }
        }
    }
    ...
    private Vector3 GetFrozenMouseAimPos()
    {
        if (mouseAim != null)
        {
            ...
            //else
            //{
            //    return mouseAim.position = BoresightPos;
            //}
        }
        else
            return transform.forward * aimDistance;
